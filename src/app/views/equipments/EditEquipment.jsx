...
const EditEquipment = () => {
  ...
  const submitEquipment = function(e){
    ...
    axios.post('/admin/equipments/update',{
      id,
      data
    }).then(data=>{
  ...
  useEffect(()=>{
    axios.get('/admin/equipments/'+id).then(response=>{
      dispatch({type:"SET_EQUIPMENT", payload: response.data.equipment});
  ...
  return (
    <Container>
      <Box className="breadcrumb">
        <Breadcrumb routeSegments={[{ name: "Equipments", path: "/admin/equipments" }, { name: "Edit Equipment" }]} />
      </Box>
      ...
      <CardRoot elevation={6}>
        ...
        <Grid container spacing={2}>
            ...
            {equipment.equipment_type == 'simple' && <Grid item xs={12} md={6}>
          ...
          <CardActions style={{justifyContent:"center"}}>
            <Button variant='contained' color='secondary' onClick={(e)=>submitEquipment(e)}>Update</Button>
          </CardActions>
      </CardRoot>
